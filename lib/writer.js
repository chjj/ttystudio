...
SGRWriter.prototype.parseCellmap = function(cellmap) {
  ...
  if (this.options.border) {
    var width = this.options.border.width;
    var color = this.options.border.color || {};
    ...
    for (var w = 0; w < width; w++) {
      ...
      for (var x = 0; x < bmp[0].length; x++) {
        tline.push({
          ...
          a: color.a || 255,
          ch: ' ',
          fg: { r: color.r, g: color.g, b: color.b, a: color.a || 255 }
        });
        bline.push({
          r: color.r,
          g: color.g,
